...
func (sq *SubjectQuery) QueryProfessorID() *TeacherQuery {
...
func (sq *SubjectQuery) QuerySubjectID() *LessonplanQuery {
	query := &LessonplanQuery{config: sq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := sq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (sq *SubjectQuery) WithProfessorID(opts ...func(*TeacherQuery)) *SubjectQuery {
	query := &TeacherQuery{config: sq.config}
	for _, opt := range opts {
		opt(query)
	}
	sq.withProfessorID = query
	return sq
}
...
func (sq *SubjectQuery) WithSubjectID(opts ...func(*LessonplanQuery)) *SubjectQuery {
	query := &LessonplanQuery{config: sq.config}
	for _, opt := range opts {
		opt(query)
	}
	sq.withSubjectID = query
	return sq
}
...
func (sq *SubjectQuery) sqlAll(ctx context.Context) ([]*Subject, error) {
	var (
		...
		_spec       = sq.querySpec()
		loadedTypes = [2]bool{
			sq.withProfessorID != nil,
			sq.withSubjectID != nil,
		}
	)
	if sq.withProfessorID != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := sq.withProfessorID; query != nil {
		ids := make([]int, 0, len(nodes))
		nodeids := make(map[int][]*Subject)
		for i := range nodes {
			if fk := nodes[i].teacher_teacher_id; fk != nil {
				ids = append(ids, *fk)
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "teacher_teacher_id" returned %v`, n.ID)
			}
			for i := range nodes {
	...
	if query := sq.withSubjectID; query != nil {
		...
		query.Where(predicate.Lessonplan(func(s *sql.Selector) {
			s.Where(sql.InValues(subject.SubjectIDColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		...
		for _, n := range neighbors {
			fk := n.subject_subject_id
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "subject_subject_id" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "subject_subject_id" returned %v for node %v`, *fk, n.ID)
			}
