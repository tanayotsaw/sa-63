...
var createPlugin = async () => {
  ...
  const {version: version2} = await fs.readJson(index.paths.resolveTargetRoot("lerna.json"));
  tasks.Task.log();
  ...
  try {
    ...
    if (await fs.pathExists(appPackage)) {
      ...
      await addPluginDependencyToApp(index.paths.targetRoot, answers.id, version2);
