  ...
  constructor() {
    this.schema = object({
      ...
      spec: object({
        ...
        implementsApis: array(string()).notRequired()
      }).required()
    });
  }
...
class GroupEntityV1alpha1Policy {
  constructor() {
    this.schema = object({
      ...
      spec: object({
        type: string().required().min(1),
        parent: string().notRequired().min(1),
        ancestors: array(string()).required(),
        children: array(string()).required(),
        descendants: array(string()).required()
      }).required()
    });
  }
  async enforce(envelope) {
...
class TemplateEntityV1alpha1Policy {
  constructor() {
    this.schema = object({
      apiVersion: string().required().oneOf(API_VERSION$3),
      kind: string().required().equals([KIND$3]),
      spec: object({
        type: string().required().min(1),
        path: string(),
...
class ApiEntityV1alpha1Policy {
  constructor() {
    this.schema = object({
      apiVersion: string().required().oneOf(API_VERSION$4),
      kind: string().required().equals([KIND$4]),
      spec: object({
        type: string().required().min(1),
        definition: string().required().min(1)
      }).required()
    });
  }
  async enforce(envelope) {
